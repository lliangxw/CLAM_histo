...
if __name__ == '__main__':
    ...
    step_size = tuple((np.array(patch_size) * (1 - patch_args.overlap)).astype(int))  ### modified, added *np.abs(patch_args.scale_prefixed)
    ...
    if data_args.process_list is None:
        if isinstance(data_args.data_dir, list):
            ...
            for data_dir in data_args.data_dir:
                # for data_dir_sigle in data_dir: #### Added 
                slides.extend(os.listdir(data_dir))  ### original code : slides.extend(os.listdir(data_dir))
        else:
    ...
    blocky_wsi_kwargs = {'top_left': None, 'bot_right': None, 'patch_size': patch_size, 'step_size': patch_size, 'scale_prefixed': patch_args.scale_prefixed, 
    'custom_downsample':patch_args.custom_downsample, 'level': patch_args.patch_level, 'use_center_shift': heatmap_args.use_center_shift} ### Added 'scale_prefixed': patch_args.scale_prefixed
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        # the actual patch size for heatmap visualization should be the patch size * downsample factor * custom downsample factor ### Added * scale_prefixed
        vis_patch_size = tuple((np.array(patch_size) * np.array(wsi_ref_downsample) * patch_args.custom_downsample * patch_args.scale_prefixed).astype(int))
        ...
        # if not os.path.isfile(h5_path_original) :  ### Modified, original code : if not os.path.isfile(h5_path) :
        ...
        # if not os.path.isfile(features_path_original): ### Modified, original code : if not os.path.isfile(features_path):
        #     file = h5py.File(h5_path, "r")
        #     features = torch.tensor(file['features'][:])
        #     torch.save(features, features_path)
        #     file.close()
        # else:  ### Added
        #     features = torch.load(features_path_original)  ### Added
        #     print("found pt files")

        # # load features 
        # ### features = torch.load(features_path)  ### Commented
        ...
        #     if not os.path.isfile(h5_path_original): ### Added
        #         file = h5py.File(h5_path, "r")
        #     else:  ### Added
        #         file = h5py.File(h5_path_original, "r") ### Added
        #     coords = file['coords'][:]
        #     file.close()
        ...
        wsi_kwargs = {'top_left': top_left, 'bot_right': bot_right, 'patch_size': patch_size, 'scale_prefixed': patch_args.scale_prefixed, 'step_size': step_size, 
        'custom_downsample':patch_args.custom_downsample, 'level': patch_args.patch_level, 'use_center_shift': heatmap_args.use_center_shift} ### Added 'scale_prefixed': patch_args.scale_prefixed
