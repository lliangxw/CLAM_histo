...
def StitchCoords(hdf5_file_path, wsi_object, scale_prefixed = -1, downscale=16, draw_grid=False, bg_color=(0,0,0), alpha=-1): ### Added scale_prefixed = -1
    ...
    print('scale_prefixed : ' + str(scale_prefixed))
    patch_size = tuple((np.array((patch_size, patch_size)) * wsi.level_downsamples[patch_level]).astype(np.int32)*np.absolute(scale_prefixed)) ###  Added *scale_prefixed
