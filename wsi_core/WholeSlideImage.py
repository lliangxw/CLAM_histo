...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        def _filter_contours(contours, hierarchy, filter_params):
            ...
            max_area = tuple((0,))  #### Added, to select only the largest connected region 
            max_cont_idx = 0  #### Added, to select only the largest connected region 
            ...
            for cont_idx in hierarchy_1:
                ...
                if a == 0: continue
                if tuple((filter_params['a_t'],)) < tuple((a,)) and max_area < tuple((a,)):  ### before modification : if tuple((filter_params['a_t'],)) < tuple((a,))
                    # filtered.append(cont_idx)  ### commented
                    # all_holes.append(holes)   ### commented

                    max_area = tuple((a,)) ### added
                    max_cont_idx = cont_idx

            filtered.append(max_cont_idx)  ### added
            holes = np.flatnonzero(hierarchy[:, 1] == max_cont_idx)  ### added
            all_holes.append(holes)  ### added
    ...
    def process_contours(self, save_path, patch_level=0, scale_prefixed = -1, patch_size=256, step_size=256, **kwargs): ### Added scale_prefixed = -1
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            asset_dict, attr_dict = self.process_contour(cont, self.holes_tissue[idx], patch_level, save_path, scale_prefixed, patch_size, step_size, **kwargs) ### Added scale_prefixed
            if len(asset_dict) > 0:
                if init:
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, scale_prefixed, patch_size = 256, step_size = 256,  #### Added scale_prefixed
        contour_fn='four_pt', use_padding=True, top_left=None, bot_right=None):  ### scale_prefixed = -1
        # start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (0, 0, self.level_dim[patch_level][0], self.level_dim[patch_level][1])  ### Original code
        start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (0, 0, self.level_dim[0][0], self.level_dim[0][1]) ### Added

        # print('patch_levels : ', self.level_downsamples)
        # print('patch_level : ', self.level_downsamples[patch_level])
        # print('patch_level_ : ', self.level_downsamples[patch_level][0])

        if scale_prefixed == -1: ### Added
            patch_downsample = (int(self.level_downsamples[patch_level][0]), int(self.level_downsamples[patch_level][1]))
        else: ### Added  
            patch_downsample = (int(self.level_downsamples[patch_level][0]*scale_prefixed), int(self.level_downsamples[patch_level][1]*scale_prefixed))  ### Added 

        ref_patch_size = (patch_size*patch_downsample[0], patch_size*patch_downsample[1])

        print("ref patch size " + str(ref_patch_size[0]))
        ...
        if len(results)>0:
            ...
            attr = {'patch_size' :            patch_size, # To be considered...
                    ...
                    'downsample':             self.level_downsamples[patch_level]*np.absolute(scale_prefixed),
                    'downsampled_level_dim' : tuple(np.array(self.level_dim[patch_level]/np.absolute(scale_prefixed))),
                    'level_dim':              self.level_dim[patch_level]/np.absolute(scale_prefixed),
                    'name':                   self.name,
                    'save_path':              save_path}
